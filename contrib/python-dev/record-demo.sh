...
comment "one node — containerd 2.2.1 (NRI v2 support)"
...
comment "nono-nri plugin config: intercepts nono-runc containers, injects nono wrap"
run "docker exec $NODE cat /etc/nri/conf.d/10-nono-nri.toml"

comment "nono binary on the node — copied by DaemonSet init container"
run "docker exec $NODE ls -lh /opt/nono-nri/nono"

comment "nono-nri DaemonSet — one pod per node, connected to NRI socket"
run "kubectl -n kube-system get pods -l app=nono-nri -o wide"

comment "plugin startup log — connected and ready"
echo ""
kubectl --context kind-nono-containerd \
  -n kube-system logs -l app=nono-nri --tail=3 | fmt_log
sleep 2
...
comment "NRI plugin intercepts CreateContainer and prepends to OCI args:"
comment "  /nono/nono wrap --profile default -- /bin/bash -c ..."
comment "nono applies Landlock, then exec()s into bash — PID 1 becomes bash"
echo ""
...
comment "plugin log: injection event for python-dev-sandbox pod"
echo ""
kubectl --context kind-nono-containerd \
  -n kube-system logs -l app=nono-nri \
  | grep "python-dev-sandbox" | tail -2 | fmt_log
sleep 1.5
