#!/usr/bin/env bash
# demo.sh — python-dev profile nono sandbox demonstration
#
# Usage:
...
LOCAL_IMAGE="nono-demo-python:latest"
...
if [[ "$RUNTIME" == "containerd" ]]; then
  ...
  docker tag "$LOCAL_IMAGE" "localhost:${REGISTRY_PORT}/nono-demo-python:latest"
  docker push "localhost:${REGISTRY_PORT}/nono-demo-python:latest" >/dev/null
  docker exec "$NODE" crictl pull "${REGISTRY_IP}:5000/nono-demo-python:latest" >/dev/null
  JOB_IMAGE="${REGISTRY_IP}:5000/nono-demo-python:latest"
...
# Remove stale jobs from previous runs
cleanup
sleep 1

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo " nono python-dev profile demo"
...
for variant in baseline sandbox; do
  echo ""
  echo "==> Applying python-dev-${variant} job..."

  # For CRI-O clusters substitute the image reference
  ...
  kubectl wait --for=condition=complete \
    job/python-dev-${variant} --timeout=120s >/dev/null

  echo ""
  echo "──── python-dev-${variant} output ────────────────────────────"
  kubectl logs "job/python-dev-${variant}"
done
