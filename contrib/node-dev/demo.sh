#!/usr/bin/env bash
# demo.sh — node-dev profile nono sandbox demonstration
#
# Usage:
...
LOCAL_IMAGE="nono-demo-node:latest"
...
if [[ "$RUNTIME" == "containerd" ]]; then
  ...
  docker tag "$LOCAL_IMAGE" "localhost:${REGISTRY_PORT}/nono-demo-node:latest"
  docker push "localhost:${REGISTRY_PORT}/nono-demo-node:latest" >/dev/null
  docker exec "$NODE" crictl pull "${REGISTRY_IP}:5000/nono-demo-node:latest" >/dev/null
  JOB_IMAGE="${REGISTRY_IP}:5000/nono-demo-node:latest"
...
# ── Cleanup helper ─────────────────────────────────────────────────────────────
cleanup() {
  kubectl delete job node-dev-baseline node-dev-sandbox \
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo " nono node-dev profile demo"
...
for variant in baseline sandbox; do
  echo ""
  echo "==> Applying node-dev-${variant} job..."
  ...
  kubectl wait --for=condition=complete \
    job/node-dev-${variant} --timeout=120s >/dev/null

  echo ""
  echo "──── node-dev-${variant} output ────────────────────────────"
  kubectl logs "job/node-dev-${variant}"
done
